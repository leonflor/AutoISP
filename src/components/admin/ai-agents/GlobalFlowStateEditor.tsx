...
import { useSaveGlobalFlowStates } from '@/hooks/admin/useGlobalFlows';
import { TOOL_CATALOG } from '@/constants/tool-catalog';
import type { AgentFlow, FlowStateInsert, TransitionRule } from '@/hooks/admin/useAgentFlows';
...
interface GlobalFlowStateEditorProps {
  flow: AgentFlow;
}

export function GlobalFlowStateEditor({ flow }: GlobalFlowStateEditorProps) {
  const saveStates = useSaveGlobalFlowStates();
  ...
  const handleSave = () => {
    ...
    saveStates.mutate({ flowId: flow.id, states: payload }, { onSuccess: () => setDirty(false) });
  };
          ...
          {states.map((state, i) => (
            <div key={i} className="border rounded-md p-3 space-y-2 bg-card overflow-hidden">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Input value={state.state_key} onChange={e => updateState(i, 'state_key', e.target.value.toLowerCase().replace(/\s+/g, '_'))} placeholder="state_key" className="h-8 text-sm font-mono min-w-0 flex-1" />
                <Input value={String(state.max_attempts)} onChange={e => updateState(i, 'max_attempts', Number(e.target.value) || 5)} className="h-8 text-xs w-16 shrink-0" title="Max tentativas" />
                <Button size="icon" variant="ghost" className="h-7 w-7 text-destructive shrink-0" onClick={() => removeState(i)}>
                  <Trash2 className="h-3.5 w-3.5" />
                </Button>
              </div>
              <Textarea value={state.objective} onChange={e => updateState(i, 'objective', e.target.value)} placeholder="Objetivo do estado" rows={2} className="text-xs resize-none" />
              <Input value={state.fallback_message} onChange={e => updateState(i, 'fallback_message', e.target.value)} placeholder="Fallback (quando max_attempts)" className="h-7 text-xs" />
              ...
              <div className="space-y-1.5">
                ...
                {state.transition_rules.map((rule, ri) => (
                  <div key={ri} className="flex items-center gap-1.5 pl-2 border-l-2 border-muted">
                    <Select value={rule.type} onValueChange={v => updateTransition(i, ri, 'type', v)}>
                      <SelectTrigger className="w-[130px] h-7 text-xs shrink-0"><SelectValue /></SelectTrigger>
                      <SelectContent>
                        {TRANSITION_TYPES.map(t => <SelectItem key={t.value} value={t.value}>{t.label}</SelectItem>)}
                      </SelectContent>
                    </Select>
                    {rule.type === 'tool_success' && (
                      <Select value={rule.tool_name || 'any'} onValueChange={v => updateTransition(i, ri, 'tool_name', v === 'any' ? undefined : v)}>
                        <SelectTrigger className="w-[130px] h-7 text-xs shrink-0"><SelectValue /></SelectTrigger>
                        <SelectContent>
                          <SelectItem value="any">Qualquer</SelectItem>
                          {TOOL_CATALOG.map(t => <SelectItem key={t.handler} value={t.handler}>{t.display_name}</SelectItem>)}
                        </SelectContent>
                      </Select>
                    )}
                    {rule.type === 'user_input' && (
                      <Input value={rule.pattern || ''} onChange={e => updateTransition(i, ri, 'pattern', e.target.value || undefined)} placeholder="Regex" className="h-7 text-xs flex-1" />
                    )}
                    <span className="text-xs text-muted-foreground shrink-0">→</span>
                    <Select value={rule.goto_state || ''} onValueChange={v => updateTransition(i, ri, 'goto_state', v)}>
                      <SelectTrigger className="w-[140px] h-7 text-xs shrink-0"><SelectValue placeholder="Destino" /></SelectTrigger>
                      <SelectContent>
                        {states.filter((_, si) => si !== i).map(s => (
                          <SelectItem key={s.state_key || `idx-${i}`} value={s.state_key}>{s.state_key || '(sem key)'}</SelectItem>
                        ))}
