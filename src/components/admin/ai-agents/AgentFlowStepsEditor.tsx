...
import { useSaveFlowSteps, type AgentFlow, type FlowStep, type FlowStepInsert } from '@/hooks/admin/useAgentFlows';
...
export function AgentFlowStepsEditor({ flow, tools, agentId }: AgentFlowStepsEditorProps) {
  ...
  const handleSave = () => {
    const payload: FlowStepInsert[] = steps
    ...
    saveSteps.mutate({ flowId: flow.id, agentId, steps: payload }, {
          ...
          {steps.map((step, i) => (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                />
                <Select
                  value={step.tool_id || 'none'}
                  onValueChange={(v) => updateStep(i, 'tool_id', v === 'none' ? '' : v)}
                >
                  <SelectTrigger className="w-[160px] h-8 text-xs">
                    <SelectValue placeholder="Sem tool" />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <Textarea
                ...
                placeholder="Instrução para o agente nesta etapa"
                rows={2}
                className="text-xs resize-none"
              />
              <div className="grid grid-cols-2 gap-2">
                <Input
                  value={step.expected_input}
                  onChange={(e) => updateStep(i, 'expected_input', e.target.value)}
                  placeholder="Input esperado (ex: CPF)"
                  className="h-7 text-xs"
                />
                <Input
                  value={step.condition_to_advance}
                  onChange={(e) => updateStep(i, 'condition_to_advance', e.target.value)}
