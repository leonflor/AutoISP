      ...
      <Card className="border-primary/50 bg-primary/5">
        ...
        <CardContent>
          <p className="text-muted-foreground">
            ...
            automação de cobranças e suporte técnico proativo. A arquitetura de agentes utiliza <code className="text-xs font-mono">agent_templates</code> → <code className="text-xs font-mono">procedures</code> (JSONB com triggers/steps) → <code className="text-xs font-mono">procedure-runner.ts</code> (com contexto determinístico v19: chaves nomeadas como confirmed_cpf_cnpj, selected_client_id, selected_contract_id) + <code className="text-xs font-mono">context-builder.ts</code> + <code className="text-xs font-mono">tool-handlers.ts</code>, 
            com 8 ferramentas canônicas (erp_client_lookup, erp_contract_lookup, erp_invoice_search, erp_pix_lookup, erp_boleto_lookup, erp_boleto_sms, erp_linha_digitavel, transfer_to_human). Com conformidade LGPD, sistema RBAC granular, backend via Supabase externo e modelo de cobrança recorrente via Asaas, a solução atende ISPs de todos os portes com trial configurável sem necessidade de cartão.
          </p>
        </CardContent>
      </Card>
    </div>
